...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-constory-black/40 data-[state=open]:animate-fade-in",
      className,
    )}
    {...props}
  />
...
>(({ className, children, hideClose, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border border-border bg-surface p-6 shadow-lg data-[state=open]:animate-fade-in max-h-[90vh] overflow-y-auto",
        className,
      )}
      {...props}
    >
